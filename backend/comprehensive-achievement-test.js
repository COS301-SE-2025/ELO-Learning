...
async function testAchievements() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/test-achievement/${testUserId}`, {
      method: 'POST',
    ...
    const submitResponse = await fetch(`${API_BASE_URL}/question/${testQuestionId}/submit`, {
      method: 'POST',
      headers: { 
    ...
    if (submitResponse.ok) {
      ...
      console.log('✅ Question submission successful:', {
        ...
        unlockedAchievements: submitData.data?.unlockedAchievements?.length || 0
      });
      ...
      console.log('❌ Question submission failed:', submitResponse.status, submitError);
    }
    
    // Test 3: Check user achievements
    console.log('\n📊 Test 3: Check User Achievements');
    const achievementsResponse = await fetch(`${API_BASE_URL}/users/${testUserId}/achievements`);
    ...
    if (achievementsResponse.ok) {
      ...
      console.log('❌ User achievements failed:', achievementsResponse.status, achievementsError);
    }
