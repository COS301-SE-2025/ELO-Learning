...
async function addSampleMatchQuestions() {
  ...
  try {
    ...
    for (let i = 0; i < sampleMatchQuestions.length; i++) {
      ...
      if (answersError) {
        console.error('Error adding answers for question', questionId, ':', answersError);
      } else {
        console.log(`✅ Added ${answersData.length} answer pairs for question ${questionId}`);
      }
    }
    ...
    if (fetchError) {
      ...
      console.log(`Found ${matchQuestions.length} match questions in database:`);
      ...
      for (const question of matchQuestions) {
        ...
        if (!answerError) {
          console.log(`- Question ${question.Q_id}: "${question.questionText.substring(0, 40)}..." (${answers.length} pairs)`);
...
// Export functions
export { addSampleMatchQuestions, addSampleMatchQuestionsWithJSON, sampleMatchQuestions, matchPairsData };
