...
async function testAchievementSystem() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        '❌ Backend achievement routes not accessible:',
        response.status,
      );
    }
  } catch (error) {
  ...
  if (sampleUser && sampleUser.length > 0) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/users/${testUserId}/achievements/perfect-session`,
        {
          method: 'POST',
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        console.log(
          '❌ Perfect Session endpoint failed:',
          response.status,
          errorData,
        );
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        console.log(
          '❌ Generic trigger endpoint failed:',
          response.status,
          errorData,
        );
      }
