...
async function simpleTest() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('📊 Found', data.categories?.length || 0, 'achievement categories');
    } else {
      console.log('❌ Server responded with status:', response.status);
    }

    console.log('\n2. Testing Perfect Session endpoint...');
    const perfectResponse = await fetch(`${API_BASE_URL}/users/96/achievements/perfect-session`, {
      method: 'POST',
    ...
    if (perfectResponse.ok) {
      ...
      console.log('❌ Perfect Session failed:', perfectResponse.status, errorData);
    }
