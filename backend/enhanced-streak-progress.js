...
async function syncAllStreakProgress() {
  ...
  try {
    ...
    const usersWithoutStreaks = allUsers.filter(
    ...
    usersWithStreaks.slice(0, 10).forEach((user, index) => {
      console.log(
        `  ${index + 1}. User ${user.id} (${user.username || 'No name'}): ${
          user.daily_streak
        } days`,
      );
    });
    ...
    for (const user of usersWithStreaks) {
      try {
        ...
        if (result.unlockedAchievements?.length > 0) {
          ...
          console.log(
            `  🏆 Unlocked ${result.unlockedAchievements.length} achievements:`,
          );
    ...
    console.log(`\n📈 Sync Summary:`);
    console.log(
      `  - ${syncedCount}/${usersWithStreaks.length} users synced successfully`,
    );
      ...
      if (userProgress && userProgress.length > 0) {
        userProgress.forEach((progress) => {
          const percentage = Math.round(
            (progress.current_value / progress.Achievements.condition_value) *
              100,
          );
          const status =
            progress.current_value >= progress.Achievements.condition_value
              ...
              : '📊';
          console.log(
            `    ${status} ${progress.Achievements.name}: ${progress.current_value}/${progress.Achievements.condition_value} (${percentage}%)`,
          );
        });
      } else {
    ...
    console.log(
      '1. ✅ All users with active streaks now have synchronized progress',
    );
    ...
    console.log(
      '4. ✅ Progress bars in frontend will now show correct percentages',
    );
  } catch (error) {
...
async function testUserStreakProgress(userId) {
  ...
  try {
    ...
    console.log(
      `User ${userId} (${user.username || 'No name'}): Current streak = ${
        user.daily_streak
      } days`,
    );
    ...
    if (progressBefore && progressBefore.length > 0) {
      progressBefore.forEach((progress) => {
        ...
        console.log(
          `  ${progress.Achievements.name}: ${progress.current_value}/${progress.Achievements.condition_value} (${percentage}%)`,
        );
      });
    } else {
