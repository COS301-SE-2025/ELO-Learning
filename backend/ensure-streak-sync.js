...
async function ensureStreakProgressSync() {
  ...
  try {
    ...
    for (const user of usersWithStreaks) {
      try {
        ...
        if (result.unlockedAchievements?.length > 0) {
          ...
          console.log(
            `📍 User ${user.id}: Unlocked ${result.unlockedAchievements.length} achievements`,
          );
        }
        ...
        if (syncedUsers % 10 === 0) {
          console.log(
            `  ✅ Synced ${syncedUsers}/${usersWithStreaks.length} users...`,
          );
        }
      } catch (error) {
    ...
    console.log(`\n📊 Final Results:`);
    console.log(
      `  - ${syncedUsers}/${usersWithStreaks.length} users synchronized`,
    );
    ...
    for (const user of sampleUsers) {
      ...
      if (progress && progress.length > 0) {
        progress.forEach((p) => {
          ...
          const status =
            p.current_value >= p.Achievements.condition_value ? '✅' : '📊';
          console.log(
            `    ${status} ${p.Achievements.name}: ${p.current_value}/${p.Achievements.condition_value} (${percent}%)`,
          );
        });
      }
    }
