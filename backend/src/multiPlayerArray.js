...
export function distributeXPFromResults(player1Results, player2Results, player1Rating, player2Rating) {
  // Validate inputs
  ...
  const [expected1, expected2] = calculateExpected(player1Rating, player2Rating)
  ...
  } else if (player1Performance.normalizedScore < player2Performance.normalizedScore) {
  ...
  const totalXP = player1Performance.totalPossibleScore + player2Performance.totalPossibleScore
  ...
  return {
    ...
    totalXPPool: totalXP,
    matchResult: matchResult1 === 1 ? 'Player 1 Wins' : matchResult1 === 0 ? 'Player 2 Wins' : 'Draw'
  }
}
