...
async function checkPersonalBestAchievement(userId, newElo, previousElo) {
  console.log(
    `🎯 Checking Personal Best Achievement for User ${userId}: ${previousElo} → ${newElo}`,
  );
  ...
  try {
    ...
    if (userError) {
      console.error(
        '❌ Error fetching user for personal best check:',
        userError,
      );
      return [];
    }
    ...
    console.log(
      `📊 Current best ELO: ${user.best_elo_rating}, New ELO: ${newElo}`,
    );
    ...
    if (user.best_elo_rating === null || newElo > user.best_elo_rating) {
      ...
      const { data: existingAchievement, error: achievementError } =
        await supabase
      ...
      if (achievementError && achievementError.code !== 'PGRST116') {
        console.error(
          '❌ Error checking existing Personal Best achievement:',
          achievementError,
        );
        return [];
      }
      ...
      if (!existingAchievement) {
        ...
        if (!unlockError || unlockError.code === '23505') {
          ...
          if (unlockError && unlockError.code === '23505') {
            console.log(
              '🔄 Personal Best achievement already unlocked (duplicate key handled gracefully)',
            );
          } else {
          ...
          console.error(
            '❌ Error unlocking Personal Best achievement:',
            unlockError.message,
          );
          return [];
        }
...
async function checkComebackAchievement(userId, newElo, previousElo) {
  console.log(
    `🎯 Checking Comeback Achievement for User ${userId}: ${previousElo} → ${newElo}`,
  );
  ...
  try {
    ...
    console.log(
      `📊 Last ELO drop: ${user.last_elo_drop}, Best ELO: ${user.best_elo_rating}`,
    );

    // If we had an ELO drop of 3+ points and now we're above our previous best
    if (
      user.last_elo_drop !== null &&
      user.last_elo_drop >= 3 &&
      user.best_elo_rating !== null &&
      newElo > user.best_elo_rating
    ) {
      console.log(
        `🏆 COMEBACK COMPLETED! Recovered from ${user.last_elo_drop} point drop and beat previous best of ${user.best_elo_rating}`,
      );
      ...
      if (achievementError && achievementError.code !== 'PGRST116') {
        console.error(
          '❌ Error checking existing Comeback achievement:',
          achievementError,
        );
        return [];
      }
      ...
      if (!existingAchievement) {
        ...
        if (!unlockError || unlockError.code === '23505') {
          ...
          if (unlockError && unlockError.code === '23505') {
            console.log(
              '🔄 Comeback achievement already unlocked (duplicate key handled gracefully)',
            );
          } else {
            ...
            return [
              {
                ...
                name: 'Comeback Kid',
                description:
                  'Recover from a 3 point drop to beat your old rating.',
                unlocked_at: new Date().toISOString(),
              },
            ];
          }
          ...
          console.error(
            '❌ Error unlocking Comeback achievement:',
            unlockError.message,
          );
          return [];
        }
      ...
      console.log(
        `📊 No comeback (drop: ${user.last_elo_drop}, new: ${newElo}, best: ${user.best_elo_rating})`,
      );
    }
 ...
 */
async function checkConsecutiveImprovementsAchievement(
  userId,
  eloChange,
  newElo,
) {
  console.log(
    `🎯 Checking Consecutive Improvements for User ${userId}: ELO change ${eloChange}, new ELO: ${newElo}`,
  );
  ...
  try {
    ...
    if (userError) {
      console.error(
        '❌ Error fetching user for consecutive improvements check:',
        userError,
      );
      return [];
    }
    ...
    console.log(
      `📊 User ${userId} stats: ${totalSessions} total sessions, consecutive improvements: ${currentCount}, last session ELO: ${lastSessionElo}`,
    );
    ...
    if (totalSessions < 2) {
      console.log(
        `🆕 New user with ${totalSessions} sessions - not tracking consecutive improvements yet`,
      );
      ...
      if (updateError) {
        console.error(
          '❌ Error updating last session ELO for new user:',
          updateError,
        );
      } else {
    ...
    if (eloChange > 0) {
      ...
      if (lastSessionElo !== null && lastSessionElo !== undefined) {
        currentCount += 1;
        console.log(
          `📈 ELO improved (+${eloChange}) from established baseline, consecutive count: ${currentCount}`,
        );
      } else {
        // First time setting up consecutive tracking for existing user
        currentCount = 1;
        console.log(
          `📈 First consecutive improvement tracking for existing user: ${currentCount}`,
        );
      }
      ...
      if (updateError) {
        console.error(
          '❌ Error updating consecutive improvements:',
          updateError,
        );
      } else {
        console.log(
          `✅ Updated consecutive improvements to ${currentCount} and last session ELO to ${newElo}`,
        );
      }
      ...
      if (currentCount >= 5) {
        console.log(
          `🏆 CONSECUTIVE IMPROVEMENTS ACHIEVED! ${currentCount} improvements`,
        );
        ...
        if (achievementError && achievementError.code !== 'PGRST116') {
          console.error(
            '❌ Error checking existing Consecutive Improvements achievement:',
            achievementError,
          );
          return [];
        }
        ...
        if (!existingAchievement) {
          ...
          if (!unlockError || unlockError.code === '23505') {
            ...
            if (unlockError && unlockError.code === '23505') {
              console.log(
                '🔄 Consecutive Improvements achievement already unlocked (duplicate key handled gracefully)',
              );
            } else {
            ...
            console.error(
              '❌ Error unlocking Consecutive Improvements achievement:',
              unlockError.message,
            );
            return [];
          }
          ...
          console.log(
            '📈 Consecutive improvements achieved but already unlocked',
          );
        }
      }
    } else if (eloChange < 0) {
      ...
      console.log(
        `📉 ELO decreased (${eloChange}), resetting consecutive count`,
      );
      ...
      if (updateError) {
        console.error(
          '❌ Error resetting consecutive improvements and tracking drop:',
          updateError,
        );
      } else {
        console.log(
          `✅ Reset consecutive improvements and tracked ${eloDrop} point drop`,
        );
      }
    } else {
      // No ELO change - update last session ELO but don't affect consecutive count
      console.log(
        `📊 No ELO change (${eloChange}), keeping consecutive count: ${currentCount}`,
      );
    ...
    console.error(
      '❌ Error in checkConsecutiveImprovementsAchievement:',
      error,
    );
    return [];
  }
 ...
 */
async function checkAllProgressAchievements(
  userId,
  newElo,
  previousElo,
  eloChange,
  ...
  console.log(
    `User: ${userId}, ELO: ${previousElo} → ${newElo} (${
      eloChange >= 0 ? '+' : ''
    }${eloChange})`,
  );
  ...
  try {
    ...
    const personalBestResults = await checkPersonalBestAchievement(
      userId,
      newElo,
      previousElo,
    );
    ...
    const comebackResults = await checkComebackAchievement(
      userId,
      newElo,
      previousElo,
    );
    ...
    const consecutiveResults = await checkConsecutiveImprovementsAchievement(
      userId,
      eloChange,
      newElo,
    );
    ...
    console.log(
      `🎯 Progress Achievement Check Complete: ${unlockedAchievements.length} achievements unlocked`,
    );
    if (unlockedAchievements.length > 0) {
      console.log(
        `🏆 Unlocked: ${unlockedAchievements.map((a) => a.name).join(', ')}`,
      );
    }
...
export {
  checkAllProgressAchievements,
  checkComebackAchievement,
  checkConsecutiveImprovementsAchievement,
  checkPersonalBestAchievement,
};
