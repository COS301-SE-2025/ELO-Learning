...
async function getAchievementIds() {
  ...
  try {
    const { data, error } = await supabase
      .from('Achievements')
      .select('id, name')
      .in('name', ['Personal Best Achieved', 'Comeback Completed', 'Consecutive Improvements']);
    ...
    if (!data || data.length === 0) {
      console.log('⚠️ No progress achievements found in database. They may need to be created.');
      return {};
    }
    ...
    console.log(`✅ Loaded ${data.length} progress achievement IDs:`, Object.keys(achievementIdCache));
...
async function unlockAchievement(userId, achievementName) {
  try {
    ...
    if (!achievementId) {
      console.log(`⚠️ Achievement "${achievementName}" not found in database - skipping unlock`);
      return null;
    }
    ...
    if (existing) {
      console.log(`📋 Achievement "${achievementName}" already unlocked for user ${userId}`);
      return null;
    }
    ...
    if (error) {
      console.error(`❌ Error unlocking achievement "${achievementName}":`, error.message);
      return null;
    }
 ...
 */
export async function checkPersonalBestAchievement(userId, newEloRating, currentBestElo) {
  try {
    // Only trigger if this is a new personal best
    if (newEloRating > currentBestElo) {
      console.log(`🎯 New personal best for user ${userId}: ${newEloRating} (was ${currentBestElo})`);
      
      const achievement = await unlockAchievement(userId, 'Personal Best Achieved');
 ...
 */
export async function checkComebackAchievement(userId, newEloRating, bestEloRating, lastEloDrop) {
  try {
    ...
    if (lastEloDrop > 0 && newEloRating >= bestEloRating) {
      ...
      if (dropAmount >= 3) {
        console.log(`💪 Comeback! User ${userId} recovered from ${lastEloDrop} to ${newEloRating} (drop was ${dropAmount} points)`);
        
        const achievement = await unlockAchievement(userId, 'Comeback Completed');
        if (achievement) {
 ...
 */
export async function checkConsecutiveImprovementsAchievement(userId, newEloRating, sessionStartElo, currentStreak) {
  try {
    ...
    if (sessionImprovement > 0) {
      ...
      newStreak += 1;
      console.log(`📈 Session improvement: +${sessionImprovement} ELO. Streak: ${newStreak}`);
      
      // Check if achievement threshold reached
      if (newStreak === 5) {
        console.log(`🔥 5 consecutive improvements achieved for user ${userId}!`);
        
        const achievement = await unlockAchievement(userId, 'Consecutive Improvements');
        if (achievement) {
          unlockedAchievements.push(achievement);
        }
      }
      ...
      console.log(`📉 Session resulted in -${Math.abs(sessionImprovement)} ELO. Streak reset.`);
      newStreak = 0;
    }
 ...
 */
export async function checkAllProgressAchievements(userId, newEloRating, sessionStartElo) {
  try {
    ...
    let { data: user, error: userError } = await supabase
      .from('Users')
      .select('id, elo_rating, best_elo_rating, last_session_elo, consecutive_improvements, last_elo_drop')
      .eq('id', userId)
      .single();
    
    if (userError) {
      console.log('⚠️ Progress tracking fields not found, trying basic fields...');
    ...
    if (Object.keys(updates).length > 0) {
      ...
      if (updateError) {
        console.error('⚠️ Error updating user progress data (fields may not exist):', updateError.message);
        console.log('💡 To fix this, run the database migration to add progress tracking fields');
