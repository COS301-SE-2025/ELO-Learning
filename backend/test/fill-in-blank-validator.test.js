    ...
    for (let blankId in correctAnswers) {
      ...
      for (const possibleAnswer of possibleAnswers) {
        ...
        if (
          ...
          if (
            mathEquivalents[studentBlank.toLowerCase()]?.includes(
              possibleAnswer.toLowerCase(),
            ) ||
            mathEquivalents[possibleAnswer.toLowerCase()]?.includes(
              studentBlank.toLowerCase(),
            )
          ) {
            isCorrect = true;
            break;
...
console.log(
  'Now fill-in-the-blank questions will work properly with mathematical expressions!',
);
