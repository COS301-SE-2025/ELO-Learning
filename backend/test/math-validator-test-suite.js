...
export class MathValidatorTestSuite {
  ...
  runTest(testName, studentAnswer, correctAnswer, expectedResult = true) {
    ...
    try {
      ...
      if (result === expectedResult) {
        ...
        console.log(
          `   Student: "${studentAnswer}" | Correct: "${correctAnswer}" | Result: ${result}`,
        );
      } else {
  ...
  testCommutativeFactoring() {
    ...
    this.runTest('Basic Commutative Factors', '(x-3)(x+3)', '(x+3)(x-3)', true);

    this.runTest('Different Variables', '(a+b)(c+d)', '(c+d)(a+b)', true);

    this.runTest('With Coefficients', '2(x+1)(x-1)', '(x-1)(x+1)*2', true);

    // Three-factor cases
    this.runTest('Three Factors', '(a+1)(b+2)(c+3)', '(c+3)(a+1)(b+2)', true);

    // Mixed with variables
    this.runTest('Variable and Factors', 'x(x+1)(x-1)', '(x-1)x(x+1)', true);
  ...
  testAlgebraicEquivalence() {
    ...
    this.runTest('Basic Algebraic', 'x^2 - 9', '(x+3)(x-3)', true);

    this.runTest('Expanded vs Factored', '(x+3)(x-3)', 'x^2 - 9', true);
  }
  ...
  testNumericalEquivalence() {
    ...
    this.runTest('Basic Numbers', '42', '42', true);

    this.runTest('Mathematical Expressions', '2 + 2', '4', true);
  }
  ...
  printSummary() {
    ...
    if (this.failedTests.length > 0) {
      ...
      this.failedTests.forEach((test) => {
        ...
        console.log(
          `    Student: "${test.student}" | Correct: "${test.correct}"`,
        );
