...
describe('Question Endpoints Integration Tests', () => {
  ...
  describe('GET /question/:id/answer', () => {
    it('should return answer for a specific question with valid auth', async () => {
      console.log(`Running /question/${testQuestionId}/answer test with auth...`);
      ...
      if (res.statusCode === 200) {
        ...
        console.warn(`Question ${testQuestionId} doesn't exist or has no correct answer`);
  ...
  describe('GET /questions/topic', () => {
    ...
    it('should return empty array for non-existent topic', async () => {
      const res = await request(app).get('/questions/topic?topic=NonExistentTopic');
  ...
  describe('GET /questions/level/topic', () => {
    it('should return questions filtered by both level and topic without auth', async () => {
      console.log(`Running /questions/level/topic?level=${testLevel}&topic=${testTopic} test...`);
      
      const res = await request(app)
        .get(`/questions/level/topic?level=${testLevel}&topic=${testTopic}`);
    ...
    it('should return 400 when level parameter is missing', async () => {
      const res = await request(app).get(`/questions/level/topic?topic=${testTopic}`);
    ...
    it('should return 400 when topic parameter is missing', async () => {
      const res = await request(app).get(`/questions/level/topic?level=${testLevel}`);
    ...
    it('should return empty array for non-existent level/topic combination', async () => {
      const res = await request(app)
        .get('/questions/level/topic?level=999&topic=NonExistentTopic');
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle special characters in topic parameter', async () => {
      const res = await request(app).get('/questions/topic?topic=Math%26Statistics');
