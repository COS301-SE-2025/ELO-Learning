...
async function fixStreakProgressBars() {
  ...
  try {
    ...
    usersWithStreaks.forEach(user => {
      console.log(`  - User ${user.id} (${user.username}): ${user.daily_streak} days`);
    });
    ...
    for (const user of usersWithStreaks) {
      ...
      try {
        ...
        console.log(`  ✅ Updated progress for ${result.unlockedAchievements?.length || 0} achievements`);
    ...
    progressData.forEach(entry => {
      ...
      userProgress[entry.user_id].push({
        ...
        percentage: Math.round((entry.current_value / entry.Achievements.condition_value) * 100)
      });
    });
    ...
    Object.entries(userProgress).forEach(([userId, achievements]) => {
      ...
      achievements.forEach(achievement => {
        console.log(`    ${achievement.achievement}: ${achievement.progress} (${achievement.percentage}%)`);
      });
    });
    ...
    console.log('2. Look for progress bars and text under Daily Streak achievements');
...
async function testStreakProgressSync() {
  ...
  try {
    ...
    console.log(`User ${testUserId} (${user.username}) current streak: ${user.daily_streak}`);
    ...
    progress?.forEach(p => {
      const percentage = Math.round((p.current_value / p.Achievements.condition_value) * 100);
      console.log(`  ${p.Achievements.name}: ${p.current_value}/${p.Achievements.condition_value} (${percentage}%)`);
    });
