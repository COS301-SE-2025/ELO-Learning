...
export default function MathInputPage() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      try {
        setLoading(true);
        const result = await getQuestionsByType('Math Input', 10);
        ...
        if (fetchedQuestions.length === 0) {
          ...
          setError('No Math Input questions found in the database.');
          return;
        }

        console.log(
          'Math Input questions fetched successfully:',
  ...
  // Success state - render the questions
  return (
