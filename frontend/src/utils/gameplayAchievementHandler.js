 ...
 */
export async function handleGameplayAchievements(
  apiResponse,
  userId,
  debug = false,
) {
  ...
  try {
    ...
    const achievements =
      apiResponse.achievements || apiResponse.newlyUnlocked || [];
    ...
    if (notYetNotified.length === 0) {
      if (debug)
        console.log('🏆 Frontend: All achievements have already been notified');
      return;
    }

    if (debug) {
      console.log(
        `🏆 Frontend: Showing notifications for ${notYetNotified.length} newly unlocked achievements:`,
    ...
    const newNotifiedIds = notYetNotified.map(
      (ach) => ach.id || ach.achievement_id,
    );
    achievementTracker.markMultipleAsNotified(newNotifiedIds);

    console.log(
      `🏆 Frontend: Successfully showed notifications for ${notYetNotified.length} achievements`,
    );
  } catch (error) {
...
export function initializeAchievementTracking(userId) {
  if (userId) {
    achievementTracker.setCurrentUser(userId);
    console.log(
      '🏆 Frontend: Achievement tracking initialized for user:',
      userId,
    );
  }
}
...
export function preventLoginNotifications(existingAchievements, userId) {
  ...
  const unlockedAchievements = existingAchievements.filter(
  ...
  if (achievementIds.length > 0) {
    ...
    console.log(
      `🏆 Frontend: Marked ${achievementIds.length} existing achievements as already notified to prevent login notifications`,
    );
  }
}
