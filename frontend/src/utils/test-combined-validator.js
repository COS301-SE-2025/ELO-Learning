...
import { isValidExpression, quickValidate, validateAnswerEnhanced, validateAnswerSync } from './answerValidator';
...
const testCases = [
  ...
  { student: 'x^2', correct: 'x*x', expected: true, description: 'Exponent notation' },
  
  // Systems of equations
  { student: 'x=3,y=4', correct: 'x=3,y=4', expected: true, description: 'System solution exact match' },
  { student: 'y=4,x=3', correct: 'x=3,y=4', expected: true, description: 'System solution different order' },
  
  // Factorization
  { student: '(x+1)(x+2)', correct: '(x+2)(x+1)', expected: true, description: 'Commutative factors' },
  
  // Multiple solutions
  { student: '2,3', correct: '2,3', expected: true, description: 'Multiple solutions' },
  { student: '3,2', correct: '2,3', expected: true, description: 'Multiple solutions different order' },
];
...
export const runValidatorTests = async () => {
  ...
  for (const test of testCases) {
    ...
    try {
      ...
      const syncResult = validateAnswerSync(test.student, test.correct, '', 'Math Input');
      console.log(`Sync result: ${syncResult} ${syncResult === test.expected ? '✅' : '❌'}`);
      
      // Test enhanced validation
      const enhancedResult = await validateAnswerEnhanced(test.student, test.correct, '', 'Math Input');
      console.log(`Enhanced result: ${enhancedResult} ${enhancedResult === test.expected ? '✅' : '❌'}`);
      
      // Test quick validation
      const quickResult = quickValidate(test.student, test.correct, '');
      console.log(`Quick result: ${quickResult} ${quickResult === test.expected ? '✅' : '❌'}`);
