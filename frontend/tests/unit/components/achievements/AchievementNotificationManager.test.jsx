...
jest.mock('../../../../src/app/ui/achievements/achievement-notification', () => {
  return function MockAchievementNotification({ achievement, show, onClose, duration }) {
    return (
      <div data-testid={`notification-${achievement.id}`}>
        <div data-testid="achievement-name">{achievement.name}</div>
        <div data-testid="achievement-description">{achievement.description}</div>
...
describe('AchievementNotificationManager', () => {
  ...
  describe('Global Function Setup', () => {
    ...
    it('dispatches achievementSystemReady event', () => {
      ...
      const readyEventCall = calls.find(call => 
        call[0].type === 'achievementSystemReady'
      )
  ...
  describe('Single Achievement Display', () => {
    it('displays a single achievement notification', async () => {
      ...
      expect(screen.getByTestId('achievement-name')).toHaveTextContent('First Steps')
      expect(screen.getByTestId('achievement-description')).toHaveTextContent('Complete your first question')
    ...
    it('handles achievement with missing data gracefully', async () => {
      ...
      expect(screen.getByTestId('achievement-name')).toHaveTextContent('Incomplete')
    })
  })
  ...
  describe('Multiple Achievement Display', () => {
    it('displays multiple achievements with staggered timing', async () => {
      ...
      const achievements = [
        global.testUtils.createMockAchievement({ id: 1, name: 'Achievement 1' }),
        global.testUtils.createMockAchievement({ id: 2, name: 'Achievement 2' }),
        global.testUtils.createMockAchievement({ id: 3, name: 'Achievement 3' })
      ]
  ...
  describe('Achievement Categories', () => {
    it('handles different achievement categories', async () => {
      ...
      for (let i = 0; i < achievementCategories.length; i++) {
        ...
        await waitFor(() => {
          expect(screen.getByTestId(`notification-${i + 1}`)).toBeInTheDocument()
        })
      }
    })
  })
