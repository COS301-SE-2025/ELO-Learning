...
async function fixDailyStreakProgress() {
  ...
  try {
    ...
    console.log(
      `Found ${streakAchievements.length} Daily Streak achievements:`,
    );
    streakAchievements.forEach((achievement) => {
      console.log(
        `  - ${achievement.name}: ${achievement.condition_value} days`,
      );
    });
    ...
    for (const user of users) {
      ...
      for (const achievement of streakAchievements) {
        ...
        if (progressValue > 0) {
          ...
          if (upsertError) {
            console.error(
              `  ❌ Error updating ${achievement.name}:`,
              upsertError.message,
            );
          } else {
            console.log(
              `  ✅ ${achievement.name}: ${progressValue}/${achievement.condition_value}`,
            );
            updatedCount++;
          }
        }
    ...
    sampleProgress?.forEach((entry) => {
      ...
      console.log(
        `  User ${entry.user_id}: ${entry.Achievements.name} = ${entry.current_value}/${entry.Achievements.condition_value} (${percentage}%)`,
      );
    });
    ...
    console.log(
      '📱 Check your frontend /achievements page - progress bars should now display correctly',
    );
  } catch (error) {
