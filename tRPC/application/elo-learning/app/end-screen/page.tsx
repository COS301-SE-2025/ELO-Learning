...
function EndScreen() {
  ...
  useEffect(() => {
    ...
    if (questionsData) {
      const questions: QuestionObject[] = JSON.parse(questionsData);
      const correctAnswers = questions.filter(
        (question: QuestionObject) => question.isCorrect === true,
      );
  ...
  const calculateXP = async () => {
    try {
      ...
      const userData: UserData = JSON.parse(decodedUserData);

      // Calculate XP earned using the same logic as TotalXP component
      const questionsData = localStorage.getItem('questionsObj');
      const questions: QuestionObject[] = questionsData
      ...
      const xpEarned = correctAnswers.reduce(
        (accumulator: number, question: QuestionObject) =>
